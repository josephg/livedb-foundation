...
Livedb.prototype.getVersion = function(cName, docName, callback) {
  this.fdb.getKey(fdb.KeySelector.lastLessThan(this._getOpKey(cName, docName)), function(err, key) {
    if (err) return callback(err);
...
Livedb.prototype.submit = function(cName, docName, opData, options, callback) {
  ...
  this.fdb.doTransaction(function(tr, callback) {
    ...
    self.snapshotDb.getSnapshot(cName, docName, function(err, snapshot) {
      ...
      tr.snapshot.getRange(fromKey, toKey, opts).toArray(function(err, ops) {
        if (err) return callback(err);
        ...
        // We need to manually add the conflict because the range was read with tr.snapshot
        tr.addReadConflictKey(self._getOpKey(cName, docName, v));
