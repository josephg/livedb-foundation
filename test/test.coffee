...
createClient = (db = new Memory()) ->
  fdb = fdbLib.open() # It would be nice to use a different database here, but we can't.

  testWrapper = {name:'test'}
  client = livedb.client {db, fdb, fdbPrefix:'/test', extraDbs:{test:testWrapper}}
  {client, fdb, db, testWrapper}
...
describe 'livedb', ->
  ...
  beforeEach (done) ->
    ...
    {@client, @fdb, @db, @testWrapper} = createClient()
